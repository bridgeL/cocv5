      ...
      <header className="chat-header">
        ...
        <div className="header-right">
          <div className="collapse-toggle">
            ...
            <button className={collapseMode === 'custom' ? 'active' : ''}
              onClick={() => setCollapseMode('custom')}>自定义</button>
            <button className={collapseMode === 'all-expanded' ? 'active' : ''}
              onClick={() => { setCollapseMode('all-expanded'); setCollapsedItems(new Set()); }}>详细</button>
          </div>
      ...
      <div className="messages" ref={messagesContainerRef}>
        {messages.map((msg, index) => {
          ...
          if (msg.type === 'think') {
            ...
            return (
              <div key={index} className={`message think ${collapsed ? 'collapsed' : ''} ${msg.isPlaceholder ? 'placeholder' : ''}`}>
                <div className="message-header" onClick={() => toggleItemCollapse(index)}>
                  {msg.isPlaceholder ? <span className="tool-call-thinking">思考中...</span> : <>思考{!msg.isComplete && <span className="tool-call-thinking">思考中...</span>} {!collapsed ? '▼' : '▶'}</>}
                </div>
                {!collapsed && !msg.isPlaceholder && <div className="message-content">{msg.content}</div>}
              </div>
            );
          }
          ...
          if (msg.type === 'report') {
            ...
            return (
              <div key={index} className={`message report ${collapsed ? 'collapsed' : ''}`}>
                <div className="message-header" onClick={() => toggleItemCollapse(index)}>
                  回答{!msg.isComplete && <span className="tool-call-thinking">回答中...</span>} {!collapsed ? '▼' : '▶'}
                </div>
          ...
          if (msg.type === 'tool') {
            return (
              <ToolCall key={index} name={msg.name} args={msg.args} result={msg.result}
                status={msg.status} collapsed={isCollapsed(index, 'tool')} onToggle={() => toggleItemCollapse(index)} />
            );
          }
      ...
      <div className="input-area">
        <div className="input-container">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)}
            onKeyPress={handleKeyPress} placeholder="输入消息..."
            disabled={connectionStatus !== 'connected'} autoComplete="off" />
          <button onClick={sendMessage} disabled={connectionStatus !== 'connected'}>发送</button>
        </div>
      </div>

      {error && <div className={`error-toast ${error ? 'show' : ''}`} onAnimationEnd={() => setError(null)}>{error}</div>}
    </div>
  );
}
